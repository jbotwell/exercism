...
public class SecurityPassMaker {
  public string GetDisplayName(TeamSupport support) {
    return support switch { Manager => "Too Important for a Security Pass",
                            Security s when (IsPriority(s)) =>
                                "Security Team Member Priority Personnel",
                            _ => support.Title };
  }

  private bool IsPriority(Security sec) {
    return !(sec is SecurityJunior || sec is SecurityIntern ||
             sec is PoliceLiaison);
  }
}
